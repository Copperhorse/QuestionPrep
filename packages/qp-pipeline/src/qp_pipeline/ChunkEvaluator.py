...
class ChunkEvaluator:
    ...
    def evaluate_chunk(self, content: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 1. FIX: Return early for empty or extremely short noise
        if len(text) < 30 or not text_lower:
            return self._result(False, "Too short/Empty", "noise", 0)

        # 2. Header-based exclusions (Metadata/Acknowledgements)
        header_text = ""
        ...
        # 5. Global metadata keyword detection
    ...
    def evaluate_chunks(self, chunks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # FIX: Ensure we explicitly pass content and metadata to each call
        for c in chunks:
            content = c.get("content", "")
            # Fallback to an empty dict if metadata is missing to avoid the TypeError
        ...
        def filter_chunks(self, chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            return [
                c for c in stats["evaluated_chunks"] if c["evaluation"]["should_use"]
            ]
